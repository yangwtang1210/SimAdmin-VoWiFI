...
use crate::{
    ...
    modem_manager::{
        ...
        background_fetch_smsc, list_current_calls, make_call, nm_set_autoconnect_pub, power_cycle_sim_for_profile_switch,
...
const VOWIFI_PROFILE_SWITCH_CONNECT_RETRY_DELAY_SECS: u64 = 1;const ESIM_SIM_IDENTITY_TIMEOUT_SECS: u64 = 3;
...
pub async fn test_automation_task_handler(
    ...
    for (group_key, mut parts) in groups {
        ...
        let total = parts
            ...
            .unwrap_or(1)
            .max(1);
        let complete = (1..=total).all(|sequence| {fn vowifi_mt_complete_group_count(outcome: &MoSmsSipOutcome) -> usize {
    let mut groups: std::collections::BTreeMap<String, Vec<&MtSmsDeliver>> =
        std::collections::BTreeMap::new();
    for deliver in &outcome.mt_deliveries {
        groups
...
pub async fn send_sms_handler(
    ...
    let mut vowifi_ready =
        vowifi_allowed && app.vowifi_runtime.snapshot().await.readiness().sms_ready;fn vowifi_restore_reason_is_soft_retry(reason: Option<&str>) -> bool {
    matches!(
        reason,
...
async fn restore_cellular_and_reset_vowifi(app: &AppState, reason: &str) -> VowifiStatusResponse {
    ...
    let _ = app.database.insert_vowifi_runtime_event(crate::db::NewVowifiRuntimeEvent {pub async fn get_vowifi_profiles_handler() -> (StatusCode, Json<ApiResponse<VowifiProfilesResponse>>)
{
    (
        StatusCode::OK,
        Json(ApiResponse::success_with_message(
...
pub async fn get_voicemail_status_handler() -> impl IntoResponse {
    (
        StatusCode::OK,
        Json(ApiResponse::<VoicemailStatusResponse>::error(#[cfg(test)]
[r command not supported in sandbox]
mod tests {
    use super::*;
